...
// use cardboard::command::{Command, DeviceId, IdentifyCommand, Reader, SetProfileCommand};
...
use cardboard::input::{KeyId, MatrixEventMessage};
use cardboard::input::{KeyMatrix, KeyboardAction};
use cardboard::profile::KeyboardEvent;
use cardboard::profile::KeyboardProfile;
use cardboard::profile::{ActionEvent, MouseEvent};
use cardboard::state::CurrentProfile;
use cardboard::storage::FlashStorage;
use cardboard::{input, matrix_task};
use defmt::*;
...
static mut MATRIX_CHANNEL: Channel<NoopRawMutex, MatrixEventMessage<SIZE>, 3> =
	Channel::<NoopRawMutex, MatrixEventMessage<SIZE>, 3>::new();
...
async fn main(spawner: Spawner) -> () {
	...
	let rows = [
		Output::new(p.PIN_28.degrade(), Level::Low),
		Output::new(p.PIN_27.degrade(), Level::Low),
		Output::new(p.PIN_26.degrade(), Level::Low),
		Output::new(p.PIN_22.degrade(), Level::Low),
		Output::new(p.PIN_21.degrade(), Level::Low),
	];

	let columns = [
		Input::new(p.PIN_20.degrade(), Pull::Down),
		Input::new(p.PIN_19.degrade(), Pull::Down),
		Input::new(p.PIN_18.degrade(), Pull::Down),
		Input::new(p.PIN_17.degrade(), Pull::Down),
		Input::new(p.PIN_16.degrade(), Pull::Down),
	];

	let matrix = KeyMatrix::new(key_ids, rows, columns, Duration::from_millis(5));
	let matrix_sender = unsafe { MATRIX_CHANNEL.sender() };

	matrix_task!(matrix_task, ROWS, COLS, 1000);
	spawner.spawn(matrix_task(matrix, matrix_sender)).unwrap();

	let device_info = DeviceInfo {
		id: DeviceId::new(Uuid::from_u128(0xd6875554_8cb4_5a57_b81f_70e91a6b7841)),
		name: "Cardboard",
		manufacturer: "cranky",
		commands: vec![], //[IdentifyCommand::default(), SetProfileCommand::default()],
	};
