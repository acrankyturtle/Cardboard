...
{
	Task<Result<ReadOnlyRentedMemory>> SendRequireResponse(SerialMessage msg);

	Task Send(SerialMessage msg);

	Task<Result<DeviceInfo>> GetDeviceInfo();
...
{
	public static async Task<Result<T>> SendRequireResponse<T>(
		this ISerialPort port,
		SerialModuleMessage msg,
		DeserializeFunc<T> deserializeResponse
	)
	{
		return await port.SendRequireResponse(msg)
			.Match<ReadOnlyRentedMemory, Result<T>>(
				response => Result<T>.Success(deserializeResponse(response.Data)),
				Result.Fail
			);
	}
}
